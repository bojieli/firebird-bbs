	...
	while (c > 0) {
		...
		if (*bptr == '/') {
			...
			case 't':
				move(0, 0);
				clrtoeol();
				sprintf(genbuf, "©Ð¶¡¡G [32m%s", chatroom);
...
char   *arg;
{
	...
	printchatline(msg);
	sprintf(msg, "[1;37m¥Ø«e%s¯¸¤W[m", (t_search(userid, NA) != NULL) ?
		"¦b" : "¤£¦b");
	printchatline(msg);
}
...
chat_date()
{
	time_t  thetime;
	time(&thetime);
	sprintf(genbuf, "[1m %s¼Ð·Ç®É¶¡: [32m%s[m", BoardName, Cdate(&thetime));
...
{
	printchatline("");
	sprintf(genbuf, "[1m¡i [36m%s [37mªº³X«È¦Cªí ¡j[m", BoardName);
	printchatline(genbuf);
