...
multi_user_check()
{
	...
	getdata(0, 0, "[1;37m±z·Q§R°£­«½Æªº login ¶Ü (Y/N)? [N][m", genbuf, 4,
		DOECHO, YEA);
	...
	while (1) {
		...
		if (strcasecmp(uid, "new") == 0) {
			...
			if (!checkpasswd(currentuser.passwd, passbuf)) {
				...
				prints("[1;31m±K½X¿é¤J¿ù»~...[m\n");
			} else {
/* °±Åv¡]Åv­­¸ò guest ¤@¼Ë¡^¨ÌµM¥i¶i¤J modify by skyo 19990602*/
/*
                if( !HAS_PERM( PERM_BASIC ) ) {
                    prints( "[1;32m¥»±b¸¹¤w°±¾÷¡C½Ð¦V [36mSYSOP[32m ¬d¸ß­ì¦][m\n" );
                    oflush();
                    sleep( 1 );
                    exit( 1 );
                }
*/
				...
				if (time(0) - currentuser.lastlogin < 60 &&
					...
					prints("¬°¤F¨¾¤î¤£¥¿±`ªº¼W¥[¨t²Î­t²ü, ½Ð¤£­n©óµu®É¶¡¤º­«ÂÐ¤W¯¸.\n");
...
notepad_init()
{
	...
	if ((time(NULL) - lastnote) >= maxsec) {
		...
		sprintf(notetitle, "[%.10s] ¯d¨¥ªO°O¿ý", ctime(&now));
...
user_login()
{
	...
	move(1, 1);
	prints("\n");
	askyn("¤µ¤Ñ¥|¤ë¤@¸¹, ±z­n§R°£¥Ø«e¨Ï¥Îªºid¶Ü", NA, NA);
	prints("\n¶}©l§R°£±b¸¹§@·~.....\n");
	sleep(1);
	prints("\n§R°£­Ó¤H«H¥ó.......done\n");
	prints("\n§R°£­Ó¤H¸ê®Æ.......done\n");
...
/*	!HAS_PERM(PERM_SYSOP) && */
	...
	if (rday < 20)
		...
		prints("§Aªº Firebird BBS ª©¥»¦³¤@ÂIÂÂ¤F, ¸Ó¤É¯Å¤F :)\n");
	else if (rday >= 30 && rday < 35)
		prints("§Aªº Firebird BBS¨t²Î¤w¸g®t¤£¦h§Ö¹L´Á¤F .. ¬O¤É¯Åªº®É­Ô¤F\n");
	else if (rday >= 35)
		prints("§Aªº BBS ª©¥»¹ê¦b¤ÓÂÂ¤F .. ½Ð°È¥²¤É¯Å¥H­×¥¿ bugs!\n");
...
check_maxmail()
{
	...
	if (currentuser.nummails > maxmail) {
		...
		if (currentuser.nummails > maxmail + 100) {
			sprintf(genbuf, "¨p¤H«H¥ó¹L¶q: %d «Ê", currentuser.nummails);
			securityreport(genbuf);
		}
	}
...
*/
		...
		prints("[1;44;33m®É¶¡:[[36m%16s[37m%14s[44;33m] Á`¤H¼Æ/¦n¤Í:[[36m%4d[33m/[1;36m%3d[33m] ±b¸¹:%-22s[m", ctime(&now),
			( is_birth(currentuser) ) ? "  ¥Í¤é­n½Ð«È  " : brdshm->date, count_users, count_friends, buf);
/*	} */
