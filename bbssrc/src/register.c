...
*/
/*
$Id: register.c,v 1.2 2000/01/18 11:22:35 edwardc Exp $
*/
...
char   *addr;
{
	...
	if ((fp = fopen(".bad_email", "r")) != NULL) {
		while (fgets(temp, STRLEN, fp) != NULL) {
			...
			/* 990802.edwardc »¡¤£©w tmp2 ¤ñ genbuf ÁÙµu«¨, ³o¼Ë´N¾×¤£¨ì¤F 
			   ©Ò¥H¥u¦n³£¸Õ¬Ý¬Ý, ¥H¨¾º|ºô¤§³½ ¦³¥i¯à user email ¬O 
			   xx@xx.net.tw, ÀÉ®×¥u¼g xx.net.tw, ÂÂ¤èªk´N¨S¿ìªk¾×ªº¦í ..*/
...
struct userec *trec;
{
	...
	if ((fin != NULL) && (fout != NULL)) {
		...
		fprintf(fout, "¥Ó½Ð¤é´Á         : %s", ctime(&trec->firstlogin));
		...
		if (!invalid_realmail(urec->userid, urec->reginfo)) {
			...
			if (dashf(buf)) {
				...
				if (i == 3) {
					...
					/* edwardc.990502 ­×¥¿·|¤@ª½­n¨D­«·sµù¥Uªº°ÝÃD */
					urec->lastjustify = time(0);
					
					/* edwardc.991019 §â e-mail ¼g¤J "¯u¹ê e-mail" */
					...
					prints("±q²{¦b°_±z±N¾Ö¦³¤@¯ë¨Ï¥ÎªÌªºÅv§Q»P¸q°È...\n");
					unlink(buf);
					mail_file("etc/smail", currentuser.userid, "Åwªï¥[¤J¥»¯¸¦æ¦C");
					pressanykey();
				}
