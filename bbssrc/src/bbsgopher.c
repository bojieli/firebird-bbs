...
char    tmpname[];
{
	...
	if ((fp = fopen(tmpname, "w")) == NULL)
		return -1;
	show_message("[5mÂà´«¤å¥ó¸ê®Æ¬°¼È¦sÀÉ");
	fprintf(fp, "¨Ó  ·½: %s\n", tmpitem->server);
	fprintf(fp, "ÀÉ  ¦W: %s(¨Ï¥Î %d °ð)\n", tmpitem->file, tmpitem->port);
	fprintf(fp, "¼Ð  ÃD: %s\n\n", tmpitem->title + 1);
	while (1) {
...
print_gophertitle()
{
	...
	prints("             [1;32mF [37m±H¦^¦Û¤vªº«H½c [32m ¡ô¡õ[37m ²¾°Ê  [32m¡÷ <Enter>[37m Åª¨ú [32m ¡ö[37m Â÷¶}");
}
printgopher_title()
{
	...
	case '=':
		{
			...
			clrtobot();
			prints("[1;44;37m");
			printdash("BBS Gopher ª«¥ó°ò¥»¸ê®Æ");
			prints("[m");
			prints("Ãþ«¬¡G%c (%s)\n", tmpitem->title[0], (tmpitem->title[0] == '0') ? "¤å¥ó" : "¥Ø¿ý");
			prints("¼ÐÃD¡G%s\n", tmpitem->title + 1);
			prints("¸ô®|¡G%s\n", tmpitem->file);
			prints("¦ì¸m¡G%s\n", tmpitem->server);
			prints("¨Ï¥Î¡G%d°ð\n", tmpitem->port);
	...
	case Ctrl('P'):
		...
		if (dashf(fname)) {
			...
			if (get_a_boardname(bname, "½Ð¿é¤J­nÂà¶Kªº°Q½×°Ï¦WºÙ: ")) {
				move(1, 0);
				sprintf(fpath, "§A½T©w­nÂà¶K¨ì %s ªO¶Ü", bname);
				if (askyn(fpath, NA, NA) == 1) {
					move(2, 0);
					postfile(fname, bname, tmpitem->title + 1, 2);
					sprintf(fpath, "[1m¤w¸gÀ°§AÂà¶K¨ì %s ªO¤F...[m", bname);
					prints(fpath);
					refresh();
					sleep(1);
				}
...
int     port;
{
	...
	inrp_by_user = YEA;
	show_message("«Ø¥ß³s½u¤¤ (Ctrl-C ¤¤Â_) ...");
...
show_gopher()
{
	...
	for (i = page; i < page + 19 && i < range; i++) {
		...
		move(i - page + 3, 0);
		prints(" %4d [[1m%9s[m] %-65s\n", i + 1, ((tmpnode->title[0] == '0') ? "[32m³s¤å" : "[33m³s¥Ø"), tmpnode->title + 1);
		refresh();
	}
}
	...
	while (1) {
		...
		if (!notreload) {
			...
			for (i = 0; i < MAXGOPHERITEMS; i++) {
				...
				sprintf(buf, "[1;3%dmÂà[3%dm´«[3%dm¸ê[3%dm®Æ[3%dm¤¤[m", (i % 7) + 1
