...
*/
/*
$Id: bbs.c,v 1.2 2000/01/15 14:39:18 edwardc Exp $
*/
...
int     mode;
{
	...
	if (search_record(BOARDS, &fh, sizeof(fh), cmpbnames, nboard) <= 0) {
		sprintf(bname, "%s °Q½×°Ï§ä¤£¨ì", nboard);
		report(bname);
		return -1;
	}
	in_mail = NA;
...
readtitle()
{
	...
	if (currBM[0] == '\0' || currBM[0] == ' ') {
		...
		sprintf(header, "ªO¥D: %s", currBM);
	}
	if (chkmail())
		strcpy(title, "[±z¦³«H¥ó]");
	else if ((bp->flag & VOTE_FLAG))
		sprintf(title, "¡° §ë²¼¤¤¡A«ö V ¶i¤J§ë²¼ ¡°");
	else
	...
	if (DEFINE(DEF_THESIS) && digestmode == 0)
		prints("[1;37;44m ½s¸¹   %-12s %6s %-38s[%4s¦¡¬Ýª©] [m\n", "¥Z µn ªÌ", "¤é  ´Á", " ¼Ð  ÃD", readmode);
	else
		prints("[1;37;44m ½s¸¹   %-12s %6s %-40s[%4s¼Ò¦¡] [m\n", "¥Z µn ªÌ", "¤é  ´Á", " ¼Ð  ÃD", readmode);
	clrtobot();
}
...
struct fileheader *ent;
{
	...
	if (FFLL == 0) {
		if (!strncmp("Re:", ent->title, 3) || !strncmp("RE:", ent->title, 3)) {
			...
			sprintf(buf, " %4d %c %-12.12s %s%6.6s[m  ¡´ %-.45s ", num, type,
				ent->owner, color, date, TITLE);
		}
	} else {
		if (!strncmp("Re:", ent->title, 3) || !strncmp("RE:", ent->title, 3)) {
			if (strcmp(ReplyPost, ent->title) == 0) {
				sprintf(buf, " [1;36m%4d[m %c %-12.12s %s%6.6s[1;36m¡E%-.48s[m "
			...
			if (strcmp(ReadPost, ent->title) == 0) {
				sprintf(buf, " [1;33m%4d[m %c %-12.12s %s%6.6s[1;33m¡D¡´ %-.45s[m "
					,num, type, ent->owner, color, date, TITLE);
			} else {
				sprintf(buf, " %4d %c %-12.12s %s%6.6s[m  ¡´ %-.45s ", num, type,
					ent->owner, color, date, TITLE);
...
struct fileheader *fhdr;
{
	...
	sprintf(buf, "<< ¥»¤å³Q %s ©ó %d/%d %d:%02d:%02d §R°£ >>",
...
int     mode;
{
	...
	if (!mode) {
		if (!strstr(quote_title, "[Âà¸ü]"))
			sprintf(buf4, "[Âà¸ü] %.70s", quote_title);
		else
...
/*    else
        sprintf(buf,"¦Û°Êµoªí¨t²Î POST '%s' on '%s'", postfile.title, currboard) ;
    report(buf) ;*/
	return 1;
}
...
char   *direct;
{
	...
	if (!SR_BMDELFLAG) {
		sprintf(genbuf, "§R°£¤å³¹ [%-.55s]", fileinfo->title);
...
notepad()
{
	...
	if ((in = fopen(tmpname, "w")) != NULL) {
		...
		if (note1[0] != 'N' && note1[0] != 'n') {
			...
			fprintf(in, "[1;34m¢y[32;44m %-48s[32m¦b [36m%.19s[32m Â÷¶}®É¯d¤Uªº¸Ü  [m\n", tmp, Ctime(&thetime));
			for (n = 0; n < i; n++) {
				if (note[n][0] == '\0')
					break;
				fprintf(in, "[1;34m¢y[33;44m %-75.75s[1;34m[m \n", note[n]);
			}
...
Goodbye()
{
	...
	*quote_file = '\0';
	...
	sprintf(spbuf, "§Aªº¿ï¾Ü¬O [[1;32m4[m]¡G");
	...
	if (choose == 1) {
		...
		sprintf(spbuf, "§Aªº¿ï¾Ü¬O [[1;32m%d[m]¡G", num_sysop + 1);
