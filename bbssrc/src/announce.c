	...
	for (n = pm->page; n < pm->page + 19 && n < pm->num; n++) {
		...
		if (a_fmode) {
			...
			if (a_fmode == 2) {
				...
				pt = localtime(&mtime);
				sprintf(fname, "[[1m%.2d[m.[1m%02d[m.[1m%02d[m]", pt->tm_year % 100 ,
...
int     nomsg;
{
	...
	int     ans = NA;
	if (!nomsg) {
		sprintf(genbuf, "½T©w±N [%-.40s] ¦s¤J¼È¦sÀÉ¶Ü", fileinfo->title);
	...
	if ((fn = fopen(buf, "r")) != NULL) {
		while (fgets(buf, sizeof(buf), fn) != NULL) {
			if (strncmp(buf, key, len) == 0 && buf[len] == ':' &&
				...
				if (netty_path[0] != '\0') {
					if (!nomsg) {
						sprintf(genbuf, "½T©w±N¸Ó¤å³¹©ñ¶i %s ¶Ü", netty_path);
					...
					if (!nomsg) {
						sprintf(genbuf, "½T©w±N¸Ó¤å³¹©ñ¶i %s ¶Ü", Importname);
						ch = askyn(genbuf, NA, YEA);
					}
					if (ch == YEA || nomsg) {
						pm.path = Importname;
...
char   *fname;
{
	...
	if (dashf(fname)) {
		...
		if ((ptr = strrchr(fname, '/')) != NULL)
			ptr++;
		sprintf(genbuf, "¨Ï¥Î Z-Modem ¶Ç°e %s ÀÉ®×¶Ü", ptr);
...
int     mode;
{
	...
	if (!valid_fname(fname)) {
		...
		sprintf(genbuf, "¨t²Î¤º¤w¦³ %s ³o­ÓÀÉ®×¦s¦b¤F...", fname);
...
MENU   *pm;
{
	...
	char    newnum[STRLEN];
	int     num, n;
	sprintf(genbuf, "½Ð¿é¤J²Ä %d ¶µªº·s¦¸§Ç: ", pm->now + 1);
...
int     paste;
{
	...
	if (!paste) {
		...
		if (*title == '\0' || *filename == '\0') {
			...
			sprintf(genbuf, "±z½T©w­nÖß¶K %s ÀÉ®×/¥Ø¿ý¶Ü", filename);
			if (askyn(genbuf, NA, YEA) == YEA) {
...
int     ch;
{
	...
	case 'f':
		pm->page = 9999;
		sprintf(genbuf, "¸ô®|¬° %s, ­n³]¬°µ·¸ô¶Ü", pm->path);
		if (askyn(genbuf, NA, YEA) == YEA) {
...
/* end of adding */
			...
			if (me.now < me.num) {
				...
				if (get_a_boardname(bname, "½Ð¿é¤J­nÂà¶Kªº°Q½×°Ï¦WºÙ: ")) {
					move(1, 0);
					sprintf(tmp, "§A½T©w­nÂà¶K¨ì %s ªO¶Ü", bname);
					if (askyn(tmp, NA, NA) == 1) {
						postfile(fname, bname, me.item[me.now]->title, 2);
						move(3, 0);
						sprintf(tmp, "[1m¤w¸gÀ°§AÂà¶K¦Ü %s ªO¤F[m", bname);
						prints(tmp);
						refresh();
						sleep(1);
					}
...
Announce()
{
	sprintf(genbuf, "%s ºëµØ°Ï¤½§GÄæ", BoardName);
